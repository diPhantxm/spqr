...
func NewDBPoolFromMultiPool(mapping map[string]*topology.DataShard, sp *startup.StartupParams, mp ShardHostsPool, tsaRecheckDuration time.Duration) *DBPool {
	dbPool := &DBPool{
		pool:              mp,
...
// TODO: add shuffle host support here
func NewDBPoolWithAllocator(mapping map[string]*topology.DataShard, startupParams *startup.StartupParams, allocator ConnectionAllocFn) *DBPool {
	dbPool := &DBPool{
